...
.. attribute:: AUTH_TOKEN_COOKIE
  ...
  For user authorization is allowed to use cookie. Default value is ``True``.
...
.. attribute:: AUTH_TOKEN_MAX_TOKEN_AGE
  ...
  Max token age if concreate token is permanent. Default value is 2 weeks.

.. attribute:: AUTH_TOKEN_COUNT_USER_PRESERVED_TOKENS

  Number of expired tokens that will be preserved for one user. Tokens are removed from the oldest one. Default value is ``20``.
...
.. attribute:: AUTH_TOKEN_RENEWAL_EXEMPT_HEADER
  ...
  HTTP header name that which causes that token expiration time will not be extended. Default value is ``'X-Authorization-Renewal-Exempt'``.
